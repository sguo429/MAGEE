...
\title{MAGEE: Mixed Model Association Test for GEne-Environment Interaction\\Version 1.2}
\date{May 26, 2022}
...
\subsubsection{Variant set tests}
...
group.file <- system.file("extdata", "SetID.withweights.txt", 
                          package = "MAGEE")
out <- MAGEE.meta(meta.files.prefix = outfile1,  
                   group.file=group.file,
                    tests=c("JV", "JF", "JD"))
@
The first argument in \textbf{MAGEE.meta} is a vector of intermediate files' prefix with length
equal to the number of studies. The argument "group.file" is the name of the group definition file. The users can choose one or more test types as "IV", "IF", "JV", "JF", and "JD" in the "tests" argument. 
...
\subsection{Version 1.2 (May 26, 2022)}
\begin{enumerate}
\item Added Meta-analysis \textbf{glmm.gei}.
\end{enumerate}
