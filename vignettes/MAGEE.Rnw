...
\title{MAGEE: Mixed Model Association Test for GEne-Environment Interaction\\Version 1.4.1}
\date{April 28, 2024}
...
\begin{enumerate}
\item Removed ARMA_DONT_PRINT_ERRORS in fitglmm.cpp (which was deprecated in RcppArmadillo version 0.11.2.1.0) and fixed minor bugs identified in the tests for link-time optimization type mismatches (thanks to: Prof. Brian Ripley).
\end{enumerate}
