...
\title{MAGEE: Mixed Model Association Test for GEne-Environment Interaction\\Version 1.4.0}
...
\subsection{Version 1.4.0 (February XX, 2024)}
\begin{enumerate}
...
\item Fixed a minor bug in the output for BGEN files in \textbf{glmm.gei}. An extra tab between "AF" and "Beta\_Marginal" was removed (thanks to: Simon Wiegrebe and Thomas Winkler).
